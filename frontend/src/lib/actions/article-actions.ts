...
export async function updateArticleAction(id: string, data: UpdateArticleRequest) {
    try {
        const cookieStore = cookies();
        const token = (await cookieStore).get('auth_token')?.value;
...
export async function summarizeArticleAction(id: string) {
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/ai/summarize/${id}`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
            },
